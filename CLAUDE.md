...
A Go HTTP service that scores transactions for fraud using brute-force exact
K-NN over a fixed reference dataset of 3 M labeled vectors (14 dimensions
each). Two API replicas + nginx load balancer, all under tight CPU/memory
caps set by the contest.
...
- `internal/dataset/` — quantized reference store. Owns the on-disk binary
  format (uint8 vectors + packed fraud bitmap) and the mmap loader. Has a
  JSON fallback path used only by tests.
- `internal/vector/` — payload struct + `Vectorize` (turns a request into a
  `[14]uint8` query vector). Pool-friendly via `Payload.Reset`.
- `internal/search/` — brute-force K-NN with int32 squared distances and
  bounds-check-elided inner loop.
...
dataset on a 2.8 GHz Xeon, GOMAXPROCS=1:

- ~34 ms / `FraudScore` call, zero allocations.
- ~29 RPS end-to-end at single-replica, no CPU throttle.
- Under the 0.4-CPU per replica cap, ~11–12 RPS per replica → ~22–24 RPS
  combined.
- Per-replica RSS ~50 MB (1.4 MB anon Go heap + 48 MB mmap'd blob).

The bottleneck is the 3 M-record linear scan. Memory headroom is
comfortable; further wins require sub-linear search (ANN, partitioning) or
accepting accuracy loss.
